...
def create_index_dataset(records_per_dataset: Union[int, Sequence[int]],
  ...
  2) Split the seed(s) into separate seeds for shuffling and preprocessing.
     ...
     - allowing users to turn on/off shuffling but still get tha same gc.SEED
  ...
  5) The gc.SEED feature contains the preprocessing seed (of the dataset) folded
     with the record_key and the epoch.
  ...
  Args:
    ...
    start_index: Where to start the input pipeline. This can be an integer,
      FIRST_INDEX or NextIndex(last_seen_index). The latter 2 to are handy for
  ...
  if is_mixture:
    ...
    def map_fn(index):
      ...
      if num_shards > 1:
        ...
        record_key += tf.cast(position_offset_per_dataset[dataset_index],
                              tf.int64)
      metadata = {
          gc.DATASET_INDEX: dataset_index,
          gc.INDEX: index,
          gc.RECORD_KEY: record_key,
      }

      # Add optional elements.
      epoch = index_in_dataset // num_records_in_dataset + 1
      if emit_epoch:
        metadata[gc.EPOCH] = epoch
      if emit_seed:
        metadata[gc.SEED] = tf.random.experimental.stateless_fold_in(
    ...
    def map_fn(index):
      ...
      metadata = {gc.INDEX: index, gc.RECORD_KEY: record_key}
