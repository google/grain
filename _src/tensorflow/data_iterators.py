...
class TfGrainDatasetIterator(dataset_iterator.DatasetIterator):
  ...
  def save(self, filename: epath.PathLike):
    ...
    # sometimes shard_index is not JSON serializable due to dtype not being int
    state["sampler"]["shard_options"]["shard_index"] = int(
        state["sampler"]["shard_options"]["shard_index"])
