...
class TfdsDataSourceTest(tf.test.TestCase, parameterized.TestCase):
  ...
  def test_repr(self):
    ...
    with mock.patch.object(data_sources, "TfArrayRecordDataSource"):
      ...
      self.assertEqual(
          repr(source),
          "TfdsDataSource(builder_directory='/path/to/data/dir', split='train', decoders=None)"
      )
      source = data_sources.TfdsDataSource(
          tfds_info,
          split="train",
      ...
      self.assertEqual(
          repr(source),
          "TfdsDataSource(builder_directory='/path/to/data/dir', split='train', decoders={'my_feature': <class 'tensorflow_datasets.core.decode.base.SkipDecoding'>})"
      )
...
class TfInMemoryDataSourceTest(tf.test.TestCase):
  ...
  def test_from_tfds(self):
    with tfds.testing.mock_data(num_examples=10):
      with mock.patch.object(tf.data, "TFRecordDataset") as tf_record:
        dummy_ds = (
            tf.data.Dataset.from_tensors(tf.constant(_EMPTY_IMAGENET_EXAMPLE)))
  ...
  def test_from_data_frame(self):
    ...
    self.assertAllClose(source[2], {
        "col1": tf.cast(2, tf.int64),
        "col2": tf.cast(2, tf.float64)
    })
    self.assertAllClose(source[3], {
        "col1": tf.cast(3, tf.int64),
        "col2": tf.cast(3, tf.float64)
    })
