...
from clu.grain.core.constants import INDEX, RECORD_KEY, DATASET_INDEX, EPOCH, SEED  # pylint: disable=g-multiple-import
from clu.grain.tf_data import index_dataset
import jax
import numpy as np
...
def create_dataset(records_per_dataset: Union[int, Sequence[int]],
  ...
  return index_dataset.create_index_dataset(
...
class IndexDatasetTest(tf.test.TestCase, parameterized.TestCase):
  ...
  def test_shuffle_other_argument_types(self, num_shards: int):
    ...
    for seed in [tuple_seed, tf_seed, jax_seed]:
      dataset = create_dataset(
          6, shuffle=True, seed=seed, num_shards=num_shards)
  ...
  def test_shuffle_and_sharding(self):
    ...
    # pyformat: enable
    dataset = create_dataset(
        6, shuffle=True, seed=(42, 73), start_index=1, shard_id=1, num_shards=2)
    values = list(dataset.take(6).as_numpy_iterator())
    # pyformat: disable
    self.assertAllEqual(values,
                        [{INDEX: 1, EPOCH: 1, RECORD_KEY: 3},
  ...
  def test_mixing_and_sharding(self):
    dataset = create_dataset([4, 6], shard_id=0, num_shards=2)
    ...
    # pyformat: enable
    dataset = create_dataset([4, 6], start_index=1, shard_id=1, num_shards=2)
    values = list(dataset.take(10).as_numpy_iterator())
    # pyformat: disable
    self.assertAllEqual(values,
  ...
  def test_seed_stable(self):
    ...
    def get_record_to_seed_map(num_shards: int = 1, **kwargs):
      assert n % num_shards == 0
      ds = create_dataset(n, emit_seed=True, num_shards=num_shards, **kwargs)
      ds = ds.take(n // num_shards)
      return {
    ...
    for i in range(2):
      seeds_3 |= get_record_to_seed_map(
          shuffle=False, start_index=i, shard_id=i, num_shards=2)
    ...
    for i in range(2):
      seeds_4 |= get_record_to_seed_map(
          shuffle=True, start_index=i, shard_id=i, num_shards=2)
  ...
  def test_invalid_start_index(self, shard_id: int, num_shards: int,
                               valid_start_indices):
    for start_index in range(10):
      if start_index in valid_start_indices:
        index_dataset.create_index_dataset(
            100,
  ...
  def test_special_start_indices(self, start_index, shard_index: int,
    ...
    ds = index_dataset.create_index_dataset(
        10,
        start_index=start_index,
        shard_id=shard_index,
        num_shards=shard_count,
