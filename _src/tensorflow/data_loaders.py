...
# Small batch size to use for `fast_warmup` option in
# _map_index_dataset_using_datasource().
_WARMUP_BATCH_SIZE = 32
_WARMUP_PARALLELISM = 2
...
class TfDataLoader:
  ...
  def __init__(self,
    ...
    Args:
      ...
      transformations: Optional list of transformations to apply before
        batching.
      batch_fn: Function to use for batching the dataset.
    """
    self.source = source
  ...
  def as_dataset(self, *, start_index: index_dataset.Index) -> tf.data.Dataset:
    ...
    ds = _map_index_dataset_using_data_source(
        self.source,
        index_ds,
        lookup_batch_size=config.tf_lookup_batch_size,
        num_parallel_calls=config.tf_lookup_num_parallel_calls)
...
def load_from_tfds(
    ...
    shard_index: int = 0,
    shard_count: int = 1,
    sharding_drop_remainder: bool = True,
  ...
  Args:
    ...
    shard_index: See TfDefaultIndexSampler.
    shard_count: See TfDefaultIndexSampler.
    sharding_drop_remainder: See TfDefaultIndexSampler.
    decoders: Additional decoding instructions for TFDS.
    transformations: List of local (stateless) transformations:
    batch_size: Optional batch size. If provided will apply TfDefaultBatchFn()
      with drop_remainder=False at the end.
  ...
  sampler = index_dataset.TfDefaultIndexSampler(
      ...
      sharding_drop_remainder=sharding_drop_remainder)
  if batch_size is not None and batch_fn is not None:
    raise ValueError("Arguments batch_size and batch_fn are mutuallyexclusive. "
                     "Only use one of them.")
  if batch_size is not None:
    batch_fn = batching.TfDefaultBatchFn(
        batch_size, drop_remainder=num_epochs is None)
  return TfDataLoader(
      source=source,
      sampler=sampler,
...
def _add_global_record_key(index_ds: tf.data.Dataset, *,
  ...
  Returns:
    The new dataset with the addod feature for the global record key.
  """
...
def _map_index_dataset_using_data_source(
    ...
    *,
    ...
    fast_warmup: bool = True,
    lookup_batch_size: int = 1024,
  ...
  Args:
    ...
    fast_warmup: If True will split up the first batch lookup into smaller
      chunks. This helps to provide initial elements but allows for a large
      `lookup_batch_size` (which is usually better for throughput).
    lookup_batch_size: Number of keys to batch into a single lookup.
      num_parallel_calls controls on how many lookups to perform in parallel.
    num_parallel_calls: Number of parallel lookups. Each lookup will record
      lookup_batch_size records. This must be a positive integer > 0 or
      tf.data.AUTOTUNE.
  ...
  if fast_warmup:
    ...
    # the end again.
    warmup_index_ds = index_ds.take(lookup_batch_size)
    index_ds = index_ds.skip(lookup_batch_size)

    warmup_dataset_cardinality = warmup_index_ds.cardinality()
    warmup_dataset = warmup_index_ds.batch(_WARMUP_BATCH_SIZE).map(
        lookup_fn, num_parallel_calls=_WARMUP_PARALLELISM).unbatch().apply(
            tf.data.experimental.assert_cardinality(warmup_dataset_cardinality))

  dataset_cardinality = index_ds.cardinality()
  dataset = index_ds.batch(lookup_batch_size).map(
      lookup_fn, num_parallel_calls=num_parallel_calls).unbatch().apply(
