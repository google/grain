...
TfDataIterator = data_iterators.TfDataIterator


class OrbaxCheckpointHandler(orbax.checkpoint.CheckpointHandler):
  """Orbax CheckpointHandler for DataIterator."""

  def save(self, directory: epath.Path, item: TfDataIterator):
    filename = directory / (
        f"process_{jax.process_index()}-of-{jax.process_count()}.json")
    item.save(filename)

  def restore(self, directory: epath.Path,
              item: TfDataIterator) -> TfDataIterator:
    if item is None:
      raise ValueError("TfDataIteratorCheckpointHandler requires `item`.")
    filename = directory / (
        f"process_{jax.process_index()}-of-{jax.process_count()}.json")
