...
class VariableSizeMultiprocessingQueue(queues.Queue):
  ...
  def __setstate__(self, state):
    super().__setstate__(state[:-3])  # pytype: disable=attribute-error
  ...
  def get(self, block: bool = True, timeout: float | None = None):
    ...
    if self._closed:  # pytype: disable=attribute-error
      raise ValueError(f"Queue {self!r} is closed")
    if block and timeout is None:
      with self._rlock:  # pytype: disable=attribute-error
        res = self._recv_bytes()  # pytype: disable=attribute-error
    else:
      if block:
        deadline = time.time() + timeout
      if not self._rlock.acquire(block, timeout):  # pytype: disable=attribute-error
        raise queue.Empty
      try:
        if block:
          timeout = deadline - time.time()
          if not self._poll(timeout):  # pytype: disable=attribute-error
            raise queue.Empty
        elif not self._poll():  # pytype: disable=attribute-error
          raise queue.Empty
        res = self._recv_bytes()  # pytype: disable=attribute-error
      finally:
        self._rlock.release()  # pytype: disable=attribute-error
    with self._resize_lock:
