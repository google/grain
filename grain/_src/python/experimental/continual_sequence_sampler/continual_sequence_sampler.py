...
def element_clip_from_index(
  ...
  if current_element_start <= index_within_epoch < next_element_start:
    ...
    return element_id, clip_id, current_element_index
...
class ContinualSequenceSampler:
  ...
  def get_element_clip_from_index(self, index: int) -> Tuple[int, int]:
    """Get the element id and clip id for the given index."""
    ...
    element_index, clip_index, self._current_element_index = (
        element_clip_from_index(
            index_within_epoch,
            epoch,
            self._start_index,
            self._element_index,
    ...
    return element_index, clip_index
...
class BatchedContinualSequenceSampler:
  ...
  def get_element_clip_from_record_key(
    ...
    element_index, clip_index, _ = element_clip_from_index(
        record_key,
        0,
  ...
  def get_element_clip_from_index(
      self,
      index: int,
  ) -> Tuple[int, int]:
    """Get the element id and clip id for the given index."""
    ...
    element_id, clip_id = sampler.get_element_clip_from_index(sub_index)
    original_element = invert_batch_idx[element_id]
    return original_element, clip_id
  ...
  def length(self) -> int:
    # In the batched sampler we set the length to be
    #   batch size * min(batch element length)
    # Although there could be valid record keys beyond this if batch elements
    # are different lengths.
    return self._batch_size * min(s.length() for s in self._batch_idx_sampler)
...
class SamplerWrapper:
  ...
  def __getitem__(self, index: int) -> record.RecordMetadata:
    element_id, clip_id = self._sampler.get_element_clip_from_index(index)
    original_start_index = self._start_index_ordered[element_id]
    rng = None
    if self._seed is not None:
      rng = np.random.Generator(np.random.Philox(key=self._seed + index))
    next_record = record.RecordMetadata(
        index=index,
        record_key=original_start_index + clip_id,
        rng=rng,
    )
    return next_record

  def record_key_to_element_and_clip(self, record_key: int) -> Tuple[int, int]:
    element_index, clip_index, _ = element_clip_from_index(
        record_key,
        0,
        self._start_index_ordered,
