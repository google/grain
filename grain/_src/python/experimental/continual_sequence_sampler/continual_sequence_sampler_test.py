...
def _get_all_metadata(
  ...
  # Mock the bisect_left function so that we can assert that it is not called in
  # any of the tests. This logarithmic function should only be called if we
  # sample indices out of order which does not happen in the tests below.
  with mock.patch.object(bisect, "bisect_left", return_value=0) as mock_bisect:
    metadata = []
    for i in gen:
      try:
        record_metadata = sampler[i]
        element_index, clip_index = sampler.record_key_to_element_and_clip(
...
class ContinualSequenceSamplerTest(parameterized.TestCase):
  ...
  @parameterized.named_parameters(
      ...
      dict(
          testcase_name="in_order_1,2,3",
          clip_map=[1, 2, 3],
          # pyformat: disable
          expected_element=[0, 1, 1, 2, 2, 2,  # epoch 0
                            0, 1, 1, 2, 2, 2],  # epoch 1
          expected_clip=[0, 0, 1, 0, 1, 2,  # epoch 0
                         0, 0, 1, 0, 1, 2,  # epoch 1
                        ],
          # pyformat: enable
      ),
  ...
  def test_no_shuffling(
    ...
    actual_metadata = _get_all_metadata(sampler, itertools.count())
    self.assert_expected_metadata(
        actual_metadata,
        list(range(sum(clip_map) * 2)),
        [i % sum(clip_map) for i in range(sum(clip_map) * 2)],
        expected_element,
        expected_clip,
    )
  ...
  def test_sharding(self):
    ...
    actual_metadata = _get_all_metadata(sampler, index_generator(shard_options))
    # pyformat: disable
    expected_element = [2, 2, 2, 2, 2,  # element 2
                        2, 2, 2, 2, 2]  # element 2
    expected_clip = [0, 1, 2, 3, 4,  # element 2
