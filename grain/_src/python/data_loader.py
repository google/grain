...
class _DataLoaderStateDatasetIterator(dataset.DatasetIterator[_T]):
  ...
  def get_state(self):
    ...
    if "iterators_in_use_states" not in dataset_state:
      ...
      iterators_in_use_indices = [0]
      iterators_in_use_states = [dataset_state]
    else:
      next_index_in_cycle = dataset_state["next_index_in_cycle"]
      iterators_in_use_indices = dataset_state["iterators_in_use_indices"]
      iterators_in_use_states = dataset_state["iterators_in_use_states"]

    workers_state = {}
    for i, state in enumerate(iterators_in_use_states):
      if state is not None:
        workers_state[iterators_in_use_indices[i]] = state
    last_worker_index = next_index_in_cycle - 1
    worker_count = len(iterators_in_use_states)
    ...
    for i in range(worker_count):
      if i not in workers_state:
        last_seen_indices[str(i)] = sys.maxsize
      else:
  ...
  def set_state(self, state):
    ...
    iterators_in_use_states = [None] * worker_count
    iterators_in_use_indices = list(range(worker_count))
    index = 0
    for worker_index in range(worker_count):
      if last_seen_indices[str(worker_index)] == sys.maxsize:
        continue
      iterators_in_use_states[index] = {
          "next_index": (
              (
      ...
      iterators_in_use_indices[index] = worker_index
      index += 1
...
class DataLoader:
  ...
  def __init__(
    ...
    for i in range(len(operations)):
      op = operations[i]
      if isinstance(op, ops.BatchOperation):
