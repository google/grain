...
class _SamplerMapDataset(grain_dataset.MapDataset[record.Record]):
...
class _OperationIterDataset(grain_dataset.IterDataset[_T]):
  ...
  def __init__(
      self,
      parent: grain_dataset.IterDataset[_T],
      operation: ops.Operation,
  ):
    super().__init__(parent)
    self.operation = operation

  def __iter__(self) -> grain_dataset.DatasetIterator[_T]:
    return _OperationDatasetIterator(self._parent.__iter__(), self.operation)


class _OperationDatasetIterator(grain_dataset.DatasetIterator[_T]):
  """Iterator that applies an operation to the dataset."""

  def __init__(
      self,
      parent: grain_dataset.DatasetIterator[_T],
      operation: ops.Operation,
  ):
    super().__init__(parent)
...
class _DataLoaderStateIterDataset(grain_dataset.IterDataset[_T]):
...
class _DataLoaderStateDatasetIterator(grain_dataset.DatasetIterator[_T]):
...
class DataLoader:
  ...
  def _create_dataset(self) -> grain_dataset.IterDataset:
...
class DataLoaderIterator(collections.abc.Iterator[_T]):
  ...
  def __init__(self, data_loader: DataLoader, state: _IteratorState):
    ...
    self._state = state
    # pylint: disable=protected-access
    self._iterator = data_loader._dataset.__iter__()
...
def _apply_transform_to_dataset(
    ...
    ds: grain_dataset.IterDataset,
) -> grain_dataset.IterDataset:
