...
class MultiprocessPrefetchIterDatasetTest(parameterized.TestCase):
  ...
  def test_validation_fails_with_sparse_map(self):
    with self.assertRaisesRegex(
        ValueError,
        'Sparse `MapDataset` transformations: .*FilterMapDataset',
    ):
      _ = prefetch.MultiprocessPrefetchIterDataset(
          ...
          .to_iter_dataset(),
          options.MultiprocessingOptions(num_workers=2),
          allow_non_elastic=False,
      )

  def test_validation_fails_with_many_to_one_iter(self):
    with self.assertRaisesRegex(
        ValueError,
        'Many-to-one `IterDataset` transformations: .*FilterIterDataset',
    ):
      _ = prefetch.MultiprocessPrefetchIterDataset(
          dataset.MapDataset.range(10)
          .to_iter_dataset()
  ...
  def test_validation_fails_with_sparse_map_and_many_to_one_iter(self):
    with self.assertRaisesRegex(
        ValueError,
        'Sparse `MapDataset` transformations: .*FilterMapDataset.*\n.*'
        'Many-to-one `IterDataset` transformations: .*BatchIterDataset',
    ):
      _ = prefetch.MultiprocessPrefetchIterDataset(
          dataset.MapDataset.range(10)
          .filter(lambda x: x > 2)
