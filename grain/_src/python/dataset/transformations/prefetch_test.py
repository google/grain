...
class PrefetchIterDatasetTest(parameterized.TestCase):
  ...
  def _assert_buffer_size(self, ds_iter, expected_size):
    if hasattr(ds_iter, '_total_buffered_count'):
      self.assertEqual(ds_iter._total_buffered_count, expected_size)
    else:
      self.assertLen(ds_iter._buffer, expected_size)
  ...
  def test_iterator_factory_selection(self):
    # Test Experiment OFF
    it = iter(self.prefetch_lazy_iter_ds)
    self.assertIs(type(it), prefetch.PrefetchDatasetIterator)
  ...
  def test_prefetch_does_not_buffer_unnecessary_elements(self):
    ...
    _ = next(ds_iter)
    self._assert_buffer_size(ds_iter, prefetch_buffer_size)
    _ = [next(ds_iter) for _ in range(14)]
    self._assert_buffer_size(
        ds_iter, len(self.range_ds) - prefetch_buffer_size
  ...
  def test_set_target_buffer_size_0_to_positive(self):
    ...
    self._assert_buffer_size(ds_iter, 2)
  ...
  def test_set_target_buffer_size_positive_to_0(self):
    ...
    self.assertEqual(next(ds_iter), 1)
    self._assert_buffer_size(ds_iter, 1)
    self.assertEqual(next(ds_iter), 2)
    self.assertEmpty(ds_iter._buffer)
...
class MultithreadPrefetchTest(parameterized.TestCase):
  ...
  def test_prefetch_data(self, num_threads: int, per_worker_buffer_size: int):
  ...
  def test_checkpoint(self, num_threads: int, step_index: int):
  ...
  def test_multiprocess_prefetch_with_default_slice_non_sequential(self):
    ds = dataset.MapDataset.source(range(10)).to_iter_dataset()
    ds_sequential_off = prefetch.multithread_prefetch(
        ds,
        num_threads=3,
        buffer_size=1,
        sequential_slice=False,
    )
    ds_sequential_default = prefetch.multithread_prefetch(
        ds,
        num_threads=3,
        buffer_size=1,
    )
  ...
  def test_multiprocess_prefetch_sequential_slice_order_from_source(self):
    ds = dataset.MapDataset.source(range(10)).to_iter_dataset()
    ds_sequential_on = prefetch.multithread_prefetch(
        ds,
        num_threads=3,
        buffer_size=1,
        sequential_slice=True,
  ...
  def test_multiprocess_prefetch_sequential_slice_order_from_range(self):
    ...
    ds_range_sequential_on = prefetch.multithread_prefetch(
        ds_range,
        num_threads=3,
        buffer_size=1,
        sequential_slice=True,
  ...
  def test_multiprocess_prefetch_sequential_slice_order_same(self):
    ...
    ds_range = dataset.MapDataset.range(10).to_iter_dataset()
    ds_source_mp = prefetch.multithread_prefetch(
        ds_source,
        num_threads=3,
        buffer_size=1,
        sequential_slice=True,
    )
    ds_range_mp = prefetch.multithread_prefetch(
        ds_range,
        num_threads=3,
        buffer_size=1,
        sequential_slice=True,
  ...
  def test_mp_context_is_set_correctly(self):
    ...
    for i, (_, context) in enumerate(results):
      self.assertEqual(context.process_index, i % num_threads)
