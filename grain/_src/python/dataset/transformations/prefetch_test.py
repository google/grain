...
class ProcessPrefetchIterDatasetTest(parameterized.TestCase):
  ...
  def test_prefetch_data(self, prefetch_buffer_size: int, warm_start: bool):
    prefetch_lazy_iter_ds = prefetch.ProcessPrefetchIterDataset(
        self.ds, prefetch_buffer_size=prefetch_buffer_size
    )
    ds = prefetch_lazy_iter_ds.__iter__()
    if warm_start:
  ...
  @parameterized.parameters([False, True])
  def test_checkpoint(self, warm_start: bool):
    ds = prefetch.ProcessPrefetchIterDataset(
        self.ds,
  ...
  def test_get_state_doesnt_start_prefetch(self):
    ...
    ds = dataset.MapDataset.source([1, 2, 3]).map(f).to_iter_dataset()
    ds = prefetch.ProcessPrefetchIterDataset(ds, prefetch_buffer_size=10)
    it = ds.__iter__()
    it.get_state()
    time.sleep(1)
    self.assertFalse(event.is_set())
  ...
  def test_parent_dataset_modifies_state(self):
    ...
    parent = TestDataset()
    ds = prefetch.ProcessPrefetchIterDataset(parent, prefetch_buffer_size=1)
    ds_iter = ds.__iter__()
  ...
  def test_fails_with_non_positive_prefetch_buffer_size(self):
    with self.assertRaisesRegex(
        ValueError, '`prefetch_buffer_size` must be greater than 0'
    ):
      prefetch.ProcessPrefetchIterDataset(self.ds, prefetch_buffer_size=0)
    with self.assertRaisesRegex(
        ValueError, '`prefetch_buffer_size` must be greater than 0'
    ):
      prefetch.ProcessPrefetchIterDataset(self.ds, prefetch_buffer_size=-1)
