...
class MultithreadPrefetchIterDatasetTest(parameterized.TestCase):
  ...
  def test_prefetch_data(self, num_workers: int, per_worker_buffer_size: int):
    prefetch_lazy_iter_ds = prefetch.MultithreadPrefetchIterDataset(
        self.ds,
  ...
  def test_checkpoint(self):
    ds = prefetch.MultithreadPrefetchIterDataset(
        self.ds,
        multiprocessing_options=options.MultiprocessingOptions(
            num_workers=2,
            per_worker_buffer_size=5,
        ),
    )
    ds_iter = ds.__iter__()
    ds_iter.start_prefetch()
  ...
  def test_get_state_doesnt_start_prefetch(self):
    ...
    ds = prefetch.MultithreadPrefetchIterDataset(
        ds,
        multiprocessing_options=options.MultiprocessingOptions(
            num_workers=2, per_worker_buffer_size=10
        ),
    )
    it = ds.__iter__()
    it.get_state()
  ...
  def test_fails_with_negative_num_workers(self):
    with self.assertRaises(ValueError):
      prefetch.MultithreadPrefetchIterDataset(
          self.ds,
          multiprocessing_options=options.MultiprocessingOptions(
              num_workers=-1
          ),
