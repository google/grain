...
class _BatchedPrefetchDatasetIterator(PrefetchDatasetIterator[T]):
  ...
  def __init__(
    ...
    if self._target_prefetch_buffer_size > 0 and self._target_num_threads > 0:
      self._batch_pushdown_size = int(
          math.ceil(
              self._target_prefetch_buffer_size / self._target_num_threads
          )
      )
    else:
      self._batch_pushdown_size = 1
  ...
  def _fill_buffer(self):
    while (
        self._total_buffered_count < self._target_prefetch_buffer_size
        and self._next_buffered_index < self._dataset_length
    ):
      batch_size = min(
          self._batch_pushdown_size,
          self._dataset_length - self._next_buffered_index,
          self._target_prefetch_buffer_size - self._total_buffered_count,
      )
      indices = list(
          range(
