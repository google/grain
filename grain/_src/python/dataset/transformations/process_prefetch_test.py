...
class MultiprocessingPrefetchTest(parameterized.TestCase):
  ...
  @parameterized.product(
      (
          dict(
              num_workers=0,
              record_state_interval=process_prefetch._RECORD_STATE_INTERVAL_S,
          ),
          dict(
              num_workers=1,
              record_state_interval=process_prefetch._RECORD_STATE_INTERVAL_S,
  ...
  def test_checkpoint(
    ...
    with mock.patch.object(
        process_prefetch, '_RECORD_STATE_INTERVAL_S', record_state_interval
    ):
  ...
  def test_set_state_twice(self):
    with mock.patch.object(process_prefetch, '_RECORD_STATE_INTERVAL_S', 0):
      ds = process_prefetch.multiprocess_prefetch(
          self.iter_ds,
          num_workers=2,
      )
  ...
  def test_fails_with_multiple_prefetches(self):
    ...
    with self.assertRaisesRegex(
      ...
      _ = process_prefetch.multiprocess_prefetch(
          ds,
          num_workers=1,
      )
