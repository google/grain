...
class MakeBatchTest(absltest.TestCase):
  ...
  def test_single_value_parallel(self):
    values = [np.asarray([1, 2, 3])]
    with concurrent.futures.ThreadPoolExecutor(
        max_workers=batch._MAX_PARALLEL_BATCH_WORKERS
    ) as executor:
      batched_values = batch._make_batch(values, parallel_executor=executor)
    self.assertEqual(batched_values.shape, (1, 3))
  ...
  def test_two_values_parallel(self):
    values = [np.asarray([1, 2, 3]), np.asarray([4, 5, 6])]
    with concurrent.futures.ThreadPoolExecutor(
        max_workers=batch._MAX_PARALLEL_BATCH_WORKERS
    ) as executor:
  ...
  def test_different_shape_parallel(self):
    values = [{"a": np.asarray([1, 2, 3])}, {"a": np.asarray([4, 5])}]
    with self.assertRaisesRegex(
        ValueError,
        "Expected all input elements to have the same structure but got:",
      ...
      with concurrent.futures.ThreadPoolExecutor(
        ...
        batch._make_batch(values, parallel_executor=executor)
  ...
  def test_different_structure_parallel(self):
    values = [{"a": np.asarray([1, 2, 3])}, {"b": np.asarray(0.5)}]
    with self.assertRaisesRegex(
        ValueError,
        "Expected all input elements to have the same structure but got:",
...
class BatchMapDatasetTest(parameterized.TestCase):
  ...
  @parameterized.named_parameters(
      dict(testcase_name="parallel", is_parallel_batch_enabled=True),
      dict(testcase_name="", is_parallel_batch_enabled=False),
  )
  def test_batch_size_2(self, is_parallel_batch_enabled: bool):
    with mock.patch.object(
        batch,
        "_is_parallel_batch_enabled",
        return_value=is_parallel_batch_enabled,
    ):
  ...
  def test_custom_batch_fn(self, is_parallel_batch_enabled: bool):
    with mock.patch.object(
        batch,
        "_is_parallel_batch_enabled",
        return_value=is_parallel_batch_enabled,
  ...
  @parameterized.named_parameters(
      dict(
          testcase_name="drop_remainder_parallel",
          drop_remainder=True,
          is_parallel_batch_enabled=True,
      ),
      dict(
          testcase_name="drop_remainder",
          drop_remainder=True,
          is_parallel_batch_enabled=False,
      ),
      dict(
  ...
  def test_batch_size_3(
      self, drop_remainder: bool, is_parallel_batch_enabled: bool
  ):
    with mock.patch.object(
        batch,
        "_is_parallel_batch_enabled",
  ...
  def test_epoch_boundaries_repeat_after_batch(
      self, drop_remainder: bool, is_parallel_batch_enabled: bool
  ):
    with mock.patch.object(
        batch,
  ...
  def test_epoch_boundaries_repeat_before_batch(
      self, drop_remainder: bool, is_parallel_batch_enabled: bool
  ):
    with mock.patch.object(
        batch,
...
class BatchIterDatasetTest(parameterized.TestCase):
  ...
  def test_batch_size_3(self, is_parallel_batch_enabled: bool):
    with mock.patch.object(
        batch,
        "_is_parallel_batch_enabled",
        return_value=is_parallel_batch_enabled,
  ...
  def test_batch_size_3_drop_remainder(self, is_parallel_batch_enabled: bool):
    with mock.patch.object(
        batch,
        "_is_parallel_batch_enabled",
        return_value=is_parallel_batch_enabled,
