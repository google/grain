...
class BatchMapDatasetTest(parameterized.TestCase):
  ...
  @parameterized.named_parameters(
      dict(
          testcase_name="parallel_jax",
          is_parallel_batch_enabled=True,
          use_jax=True,
      ),
      dict(
          testcase_name="parallel_no_jax",
          is_parallel_batch_enabled=True,
          use_jax=False,
      ),
      dict(testcase_name="jax", is_parallel_batch_enabled=False, use_jax=True),
      dict(
          testcase_name="no_jax", is_parallel_batch_enabled=False, use_jax=False
      ),
  )
  def test_batch_size_2(self, is_parallel_batch_enabled: bool, use_jax: bool):
    with mock.patch.dict(
        sys.modules, {"jax": sys.modules["jax"] if use_jax else None}
    ):
      importlib.reload(batch.tree_lib)
      with experiment_mock_utils.mocked_experiment_context(
          "EXP_parallel_batch" if is_parallel_batch_enabled else ""
      ):
...
class BatchIterDatasetTest(parameterized.TestCase):
  ...
  def test_batch_size_3(self, is_parallel_batch_enabled: bool, use_jax: bool):
    with mock.patch.dict(
        sys.modules, {"jax": sys.modules["jax"] if use_jax else None}
    ):
      importlib.reload(batch.tree_lib)
  ...
  def test_batch_size_3_drop_remainder(
      self, is_parallel_batch_enabled: bool, use_jax: bool
  ):
    with mock.patch.dict(
        sys.modules, {"jax": sys.modules["jax"] if use_jax else None}
    ):
