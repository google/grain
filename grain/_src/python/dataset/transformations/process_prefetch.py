...
def _clear_buffer(q: queues.Queue[Any]):
  while True:
    try:
      element, _, _ = q.get_nowait()
...
def _clear_set_state_queue(q: queues.Queue[Any]) -> bool:
  try:
    q.get_nowait()
    return True
  except queue.Empty:
...
def _put_dataset_elements_in_buffer(
  ...
  try:
    ...
    _clear_set_state_queue(set_state_queue)
    buffer.put((None, None, e))
    return
  _clear_buffer(buffer)
  _clear_set_state_queue(set_state_queue)
...
class _ProcessPrefetchDatasetIterator(dataset.DatasetIterator[T]):
  ...
  def _clear_buffer(self):
    _clear_buffer(self._buffer)

  def _clear_set_state_queue(self):
    if _clear_set_state_queue(self._set_state_queue):
      self._set_state_count -= 1
