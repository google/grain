...
"""Implements LazyDataset elements prefetching with processes."""
  ...
  try:
    ...
    while not should_stop.is_set():
      ...
      try:
        ...
        continue
      element = shared_memory_array.copy_struct_to_shm(
          element, min_size=min_shm_size
      )
...
class _ProcessPrefetchDatasetIterator(dataset.DatasetIterator[T]):
  ...
  def start_prefetch(self) -> None:
    ...
    self._prefetch_process = self._process_ctx.Process(
        ...
        kwargs=dict(
            ...
            pickled_ds=_serialize_dataset(self._iter_parent),
            buffer=self._buffer,
  ...
  def __next__(self):
    ...
    with timer:
      ...
      while True:
        ...
        if isinstance(element, _SetStateIsDone):
          ...
          shared_memory_array.open_struct_from_shm(element)
      if err is not None:
        self._stop_prefetch()
    ...
    with self._stats.record_self_time(offset_ns=timer.value()):
      ...
      return shared_memory_array.open_struct_from_shm(element)
...
def multiprocess_prefetch(
  ...
  for i in range(num_workers):
    ...
    worker_ds = _MpContextIterDataset(
        worker_ds,
    ...
    worker_ds = ProcessPrefetchIterDataset(
        ...
        else functools.partial(worker_init_fn, i, num_workers),
    )
