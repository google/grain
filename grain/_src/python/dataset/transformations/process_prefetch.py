      ...
      if set_state_request_count.value > 0:
        with set_state_request_count.get_lock():
          ...
          if not multiprocessing_common.add_element_to_queue(  # pytype: disable=wrong-arg-types
              (_SetStateIsDone(), None, None, None), buffer, should_stop.is_set
          ):
            continue
...
class _ProcessPrefetchDatasetIterator(dataset.DatasetIterator[T]):
  ...
  def get_state(self) -> StateT:
    if self._state is not None:
      ...
      # Get the state corresponding to the new `next_index` if `get_state` is
      # called after `set_next_index` and before the next `__next__` call.
