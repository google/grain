...
class ConcatThenSplitIterDatasetTest(parameterized.TestCase):
  ...
  def test_non_first_full_length_sequence_passed_through_immediately(self):
    ds = dataset.MapDataset.source([
        {"observation": np.arange(5)},
        {"observation": np.arange(5)},
        # When the buffer is empty and we encounter a full-length sequence,
        # we should pass it through immediately.
        {"observation": np.arange(10)},
        {"observation": np.arange(5)},
        {"observation": np.arange(5)},
    ]).to_iter_dataset()
  ...
  def test_first_full_length_sequence_not_passed_through_immediately(self):
    ds = dataset.MapDataset.source([
        # When the *first* buffer is empty and we encounter a full-length
        # sequence, we should wait until the buffer is full before passing it
        # through. This is a weird and confusing behavior, but it's what we've
        # done historically so it's hard to change!
        {"observation": np.arange(10)},
        {"observation": np.arange(5)},
        {"observation": np.arange(5)},
    ]).to_iter_dataset()
