...
def _find_shard_file(
  ...
  pattern = f"shard_state_{shard_index}.json"
...
def save_elastic_iterator(
  ...
  if not isinstance(ds_iterator_states, dict):
    raise ValueError(
        "Expected ds_iterator_states to be a dict, but got"
        f" {type(ds_iterator_states)}"
    )
