...
def _find_shard_file(
  ...
  if not found_files:
    raise ValueError(
        f"No shard state files found in {directory} for shard {shard_index} of"
        f" {total_num_shards}"
    )
  if len(found_files) > 1:
    raise ValueError(
        f"Multiple shard state files found in {directory} for shard"
        f" {shard_index} of {total_num_shards}"
    )
...
def save_elastic_iterator(
  ...
  num_dataset_shards = item.num_dataset_shards
  for idx, host_iterator_state in ds_iterator_states.items():
    shard_state = json.dumps(host_iterator_state, indent=4)
    filename = directory / f"shard_state_{idx}-of-{num_dataset_shards}.json"
...
def restore_elastic_iterator(
  ...
  iterator_states = {}
  while shard_index < num_dataset_shards:
    filename = _find_shard_file(directory, shard_index, num_dataset_shards)
    state = filename.read_text()
    state = json.loads(state)
    iterator_states[shard_index] = state
    shard_index += shard_count
  item.set_state({"ds_iterator_states": iterator_states})
...
class ElasticCheckpointHandler:
  ...
  def save(
    ...
    """Saves the given iterator to the checkpoint in `directory`."""
    items = item or args.item
    if isinstance(items, elastic_iterator.ElasticIterDatasetIterator):
      items = [items]
