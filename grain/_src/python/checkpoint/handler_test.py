...
class CheckpointHandlersTest(absltest.TestCase):
  ...
  def test_data_loader_checkpoint_save_and_restore(self):
    ...
    mngr = ocp.CheckpointManager(tmpdir)
    mngr.save(
        0,
        args=handler.CheckpointSave(
            self._create_data_loader_iter_to_checkpoint()
        ),
    )
    mngr.wait_until_finished()
    ds = self._create_data_loader()
    ds_iter = iter(ds)
    mngr.restore(0, args=handler.CheckpointRestore(ds_iter))
  ...
  def test_dataset_checkpoint_save_and_restore(self):
    ...
    mngr.save(
        0,
        args=handler.CheckpointSave(self._create_dataset_iter_to_checkpoint()),
    )
    mngr.wait_until_finished()
    ds = self._create_dataset()
    ds_iter = iter(ds)
  ...
  def test_composite_checkpoint_save_and_restore(self):
    tmpdir = f"{self.create_tempdir().full_path}/checkpoint"
    mngr = ocp.CheckpointManager(tmpdir, item_names=("state", "ds_iter"))
    mngr.save(
        0,
        args=ocp.args.Composite(
            state=ocp.args.StandardSave({"values": [0]}),
            ds_iter=handler.CheckpointSave(
                self._create_data_loader_iter_to_checkpoint()
            ),
        ),
    )
    ...
    mngr.restore(
        0,
        args=ocp.args.Composite(
            state=ocp.args.StandardRestore({"values": [0]}),
            ds_iter=handler.CheckpointRestore(ds_iter),
        ),
    )
    self._assert_restored_data_loader_iter(ds_iter)
